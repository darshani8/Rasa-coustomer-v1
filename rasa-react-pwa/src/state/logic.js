// AUTO-GENERATED from the reference DCLogic. Handlers/data/renderVals are verbatim;
// only lifecycle, initial state, and image sources were adapted for the PWA.
const ASSET = ((import.meta && import.meta.env && import.meta.env.BASE_URL) || '/') + 'img/';

export const initialState = { screen: 'home', vendorId: 'artiste', tab: 'Menu', cart: {}, payMethod: 'visa', qSec: 765, bankOpen: false, orderFilter: 'all', orderSort: 'recent', sortOpen: false, offerFilter: 'All Offers', selectedOffer: null, couponInput: '', foodCat: 'All', catSort: 'wait', searchQuery: '', supportTopic: 'orders', faqOpen: null, chatInput: '', chatMsgs: null, ticketCat: 'Order issue', ticketText: '', address: { label: 'Home', line1: '12th Main Rd, HAL 2nd Stage', line2: 'Indiranagar', city: 'Bengaluru, Karnataka', pin: '560038' }, notifs: { push: true, orders: true, offers: false, email: true, sms: false }, language: 'English', location: 'Indiranagar, BLR', dietFilter: 'all', dietMenuOpen: false, queueSheet: false, parkSheet: false, parkDay: 'today', parkQty: 1, parkSlot: null, street: null, streetFilter: 'All', otp: ['', '', '', ''] };

class RasaLogic {
  constructor(getState, setState, props) { this._get = getState; this._set = setState; this.props = props || {}; }
  get state() { return this._get(); }
  setState(u, cb) { this._set(u, cb); }
  ...
  go(screen) { this.setState({ screen, queueSheet: false, parkSheet: false }); }
  ...
  renderVals() {
    ...
    const truckPhoto = { camion: ASSET+'truck-camion.jpg', artiste: ASSET+'truck-artiste.jpg', punjab: ASSET+'truck-punjab.jpg', chaat: ASSET+'truck-chaat.jpg', handi: ASSET+'truck-handi.jpg', artisan: ASSET+'truck-artisan.jpg', chai: ASSET+'truck-chai.jpg', saigon: ASSET+'truck-saigon.jpg' };
...
export function computeVals(getState, setState, props) {
  return new RasaLogic(getState, setState, props).renderVals();
}
