...
export default function Login() {
  ...
  const doGoogleLogin = useStore((st) => st.doGoogleLogin);
    ...
    <div style={s('animation:rasaFade .35s ease;display:flex;flex-direction:column;min-height:100%')} data-screen-label="Sign in">
      <div style={s('flex:1;display:flex;flex-direction:column;justify-content:center;padding:0 26px')}>
        ...
        <div style={s('display:flex;flex-direction:column;gap:11px')}>
          <button onClick={() => void doGoogleLogin()} style={s("width:100%;display:flex;align-items:center;justify-content:center;gap:10px;background:#fff;color:#3B2630;border:1px solid #ECE6DB;border-radius:var(--radL,16px);padding:14px;font:700 12.5px var(--display,'Space Grotesk');cursor:pointer")}>
